...
export class Particle {
  constructor(pos) {
    ...
    this.textArr = RANDOM_TEXT.split(""); //문자열을 어레이로 변환해서 넣는다.
    this.cur = 0;
  ...
  draw(ctx, t) {
    ...
    const red = ((this.rgb >> 16) & 0xff) | 0; //hex to rgb변환 식
    ...
    ctx.fillText(
      ...
      this.y + (fontSize - textPos.actualBoundingBoxAscent)
    );
  }

  shuffle(arr) {
    return arr.sort(() => Math.random() - 0.5); //랜덤 배열을 만드는 방법, 함수의 리턴값이 음수또는 양수로 항상 랜덤이기 때문에 배열이 뒤죽박죽 섞인다.
    //즉 두 인수를 비교하지 못해서 수가 뒤섞이는 것이다.
  }
}
